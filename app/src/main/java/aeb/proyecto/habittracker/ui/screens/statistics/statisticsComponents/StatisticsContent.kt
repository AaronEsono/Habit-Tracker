...
fun StatisticsContent(
    ...
    LaunchedEffect(true) {
        statisticsViewModel.getDailyHabits()
    }
    ...
    val habitSelected = statisticsViewModel.selectedDailyHabit.collectAsState().value
    ...
    LazyColumn(modifier = Modifier.fillMaxSize()) {
        ...
        stickyHeader {
            LazyRow(
                ...
                items(habits) {
                    RadioButtonStatistics(it, habitSelected) { habit ->
                        if (habit != habitSelected) {
                            statisticsViewModel.setSelectedHabit(habit)
                        }
                    }
                }
            }
        ...
        item {
            AnimatedContent(targetState = habitSelected, label = "",
                transitionSpec = {
                    slideInVertically { height -> -height } togetherWith slideOutVertically { height -> height }
                }) { habitSelected ->
                habitSelected?.let {
                    StatisticsHeader(habitSelected)
                }
            }
        }
        ...
        item {
            AnimatedContent(targetState = statisticsState, label = "",
                transitionSpec = {
                    ...
                    Column {
                        ...
                        Row {
                            StatisticsCard(
                                ...
                                statisticsState.actualStreak,
                                title = R.string.statistics_screen_streak
                            )
                            StatisticsCard(
                                modifier = Modifier.weight(1f),
                                statisticsState.bestStreak,
